		...
		$.ajax({
			...
			success : function(json) {
				...
				if (json.length > 0) {
					...
					$("#cod_estado option[value='" + json[3] + "']").attr(
							"selected", true);

					//Busca as cidades conforme o estado selecionado
					getCidade(urlCidades, json[3], 'cod_cidade',
							'combo_cidade', json[2]);
				...
				} else { //Cai aqui quando cep não foi encontrado
...
/** Função JSON para Enviar Formulário
 * @param String url
 * @param String form
...
// quando login no cms é feito através do site:
/*function esqueceu(url, txt_login, classe, msg_erro, estrategia, msg_esqueceu, form)
 ...
 //altera o valor para aguarde para não poder clicar
 ...
 //se não tiver o login entra aqui
 else if(json[0] == "0")
 {
 limpaDiv(classe, msg_esqueceu);
 document.getElementById(msg_erro).innerHTML = json[1];
 ...
 {
 limpaDiv(classe, msg_esqueceu);
 document.getElementById(msg_erro).innerHTML = "Preencha o seu nome de usuário para recuperar a senha";
...
/*function getCidade(url,combo_value,combo_name){
 ...
 //Recebe uma instancia da div que receberá os dados da combo atualizados			
